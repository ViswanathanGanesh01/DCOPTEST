...
T_FREEZE_C = 3.0       # Freeze Protection Temperature (°C)
...
def run_single_simulation(args):
    ...
    try:
        ...
        start_vals = {}
        for var_name in ['Arch', 'architecture', 'AirSupSet', 'AirRetSet', 'LiqSupSet', 'LiquidRetSet', 'CWSupSet', 'CWRetSet', 'TFreeze', 'TApp', 'TCHWSupSet', 'TDryBulb', 'TWetBulb', 'ITAir', 'ITLiq']:
        ...
        # Dynamic solver tolerance helper (dry chiller and liquid configurations require relaxed tolerances to resolve numeric stiffness/chattering)
        dynamic_tol = 0.1 if int(arch_value) in [8, 9, 11, 12] else 1e-2
        #dynamic_tol = 1e-2
...
def plot_equipment_energy(df):
    ...
    data.rename(columns={"Chiller": "Chiller", "CoolingTowerFan": "CT Fan", "PumpCW": "CW Pump", "PumpCHW": "CHW Pump", "Fan": "AHU Fan", "D2CPump": "D2C Pump"}).plot(kind='bar', stacked=True, ax=ax, width=0.75, edgecolor='black', alpha=0.8)
